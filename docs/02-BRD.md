...
| Status | **Draft for Approval** (revisi v1.5 — menyelaraskan seluruh angka harga dan hitungan katalog dengan lembar "Product" bisnis plan, dan merapikan riwayat revisi yang sempat tertinggal) |
...
|---|---|---|---|---|---|
| **US-02** | Memfilter katalog berdasarkan kategori dan tier | Must | Should — Fase 1b | Katalog Fase 1 berisi 11 halaman produk dalam dua kategori, ditambah lini daftar 100 gram. Pada ukuran ini, filter berfaset tidak memperbaiki penemuan produk secara berarti, tetapi menambah state UI, sinkronisasi URL, dan permukaan pengujian. Biayanya tidak sebanding dengan manfaatnya sebelum katalog tumbuh | Katalog Fase 1a dikelompokkan menjadi bagian Single Origin dan Houseblend dengan navigasi lompat di bagian atas (FR-03), sehingga pengunjung tetap sampai ke kelompok yang relevan dalam satu ketukan |
...
| **FR-52** | Katalog Kopi 100 gram | Halaman katalog menayangkan lini kedua berisi 18 kopi dalam kemasan 100 gr, lengkap dengan nama dan harga per biji, dan seluruhnya dapat ditambahkan ke keranjang. Lini ini **tidak** memakai `Tier`, sehingga `BR-09` tidak berlaku padanya. Tidak ada halaman produk per biji dan tidak ada kolom asal, proses, ketinggian, varietas, maupun catatan rasa selama owner belum menyerahkan datanya; halaman menyatakan ketiadaan itu secara terbuka. Slug lini ini tidak boleh bertabrakan dengan slug produk 200 gram | Must | 1a | KD-07 |

| **FR-53** | Pengelolaan katalog oleh owner | Harga single origin per tier, harga houseblend per kg, status jual per produk, dan seluruh lini 100 gram dapat diubah owner lewat Google Sheet tanpa menyentuh kode, lalu diterbitkan lewat satu aksi manual atau jadwal harian. Sinkronisasi wajib **gagal tertutup**: setiap tab hilang, harga hilang, harga di luar batas kewarasan, kunci tak dikenal, slug bentrok, atau daftar kosong membatalkan penerbitan tanpa mengubah berkas apa pun, sehingga katalog terakhir yang sah tetap tayang. Data non-angka — asal, proses, ketinggian, varietas, catatan rasa, foto — TIDAK termasuk | Must | 1a | KD-08 |

**Rekapitulasi (v1.5):** 53 FR. Fase 1a memuat **38 FR**; Fase 1b memuat **13 FR** (FR-04, FR-05, FR-06, FR-13, FR-14, FR-15, FR-30, FR-32, FR-33, FR-34, FR-39, FR-40, FR-50). FR-49 berpindah dari Fase 1b ke Fase 1a, kecuali bagian `FAQPage` yang mengikuti FR-32 di Fase 1b. **FR-51 ditambahkan pada v1.2** atas keputusan CEO 8 September 2026 (KD-05, `00b-ceo-decisions.md`); ia tidak berasal dari kebutuhan awal mana pun, sehingga tidak menelusur ke user story yang sudah ada. **FR-52 ditambahkan pada v1.3** atas keputusan CEO 8 September 2026 (KD-07), yang juga membatasi jangkauan `BR-09` menjadi lini 200 gram saja. **FR-53 ditambahkan pada v1.4** atas keputusan CEO 8 September 2026 (KD-08); sejak itu angka pada `BR-09`, `BR-14`, `BR-15`, dan `BR-16` dipelihara owner lewat spreadsheet, sementara aturannya sendiri tetap di BRD. **v1.5 tidak menambah FR mana pun** — ia hanya menyelaraskan angka dan mencabut aturan harga yang sudah dibatalkan.
...
- [ ] Seluruh produk tayang di katalog: **11 halaman produk** — tiga lini houseblend (BOLD, BRIGHT, Full Robusta) dan **delapan** single origin, termasuk Sindoro. Ditambah lini Katalog Kopi 100 gram (FR-52) yang berdiri sendiri. **Jumlah total varian jual sengaja tidak ditulis sebagai angka di sini**: ia sedang berubah mengikuti pemisahan houseblend menjadi dua ukuran kemasan (BR-13). Angka yang berlaku adalah `EXPECTED_*` pada `web/src/data/validate.ts`, yang menggagalkan build bila katalog diam-diam bertambah atau berkurang — periksa ke sana, jangan ke dokumen ini.
...
**Syarat masuk:** BRD v1.5 disetujui CEO; foto produk minimal satu per produk tersedia atau placeholder disetujui; domain titikasalkopi.id aktif. Jam balas WhatsApp **sudah ditetapkan** lewat keputusan KD-03 (setiap hari 08.00–21.00 WIB), demikian pula aturan 3 pack (KD-01) dan kelipatan 0,5 kg (KD-02), sehingga tidak ada lagi keputusan terbuka yang menahan tanggal rilis.
...
| **Kemasan mini 100 gr** | Kemasan kecil pada lini single origin 200 gr, dari kolom "Single Origin - Mini Packs" (BR-08). Berbeda dari **Katalog Kopi 100 gram** (FR-52, KD-07), yang merupakan lini terpisah berisi biji lain dengan harga per biji — keduanya sama-sama 100 gram dan karena itu wajib dibedakan setiap kali disebut |
