...
| **DEF-06** | **Major** | **JavaScript awal beranda = 181 KB terkompresi**, melewati anggaran NFR-03 sebesar 150 KB. Delapan chunk dirujuk sebagai `<script>` pada beranda: 71 KB, 43 KB, 39 KB, 10 KB, 8 KB, 6 KB, 4 KB, 2 KB setelah gzip. Berat muat pertama total ~264 KB, masih jauh di bawah anggaran 600 KB — jadi yang terlampaui adalah anggaran JavaScript-nya saja | `gzip -9` setiap chunk yang dirujuk `out/index.html`, lalu jumlahkan | NFR-03 | **FE / Arsitek** | **TERBUKA.** Diukur dengan gzip lokal, bukan brotli yang dipakai Vercel; brotli biasanya 10–15% lebih kecil, yang menempatkan angkanya di kisaran 158–165 KB — **masih di atas anggaran**. Wajib diukur ulang pada panel Network setelah deployment (TC-197) sebelum NFR-03 dinyatakan lulus atau angkanya direvisi CEO |
| **DEF-07** | Minor | `src/app/opengraph-image.png` tidak ada. Karena kesepuluh produk juga belum berfoto, **tidak ada satu pun halaman** yang memiliki gambar pratinjau saat tautannya dibagikan. FR-46 belum penuh | Build, lalu cari `og:image` pada HTML | FR-46, NFR-10 | **Owner / desainer** | **TERBUKA.** Dilaporkan sendiri oleh FE (D-FE-02) dan BE, dan **saya konfirmasi**. Aset desain 1200 x 630; tidak boleh dibuat secara terprogram karena berarti mengarang identitas visual |
| **DEF-08** | Minor | Kesepuluh produk bernilai `image: null`; seluruh katalog tayang dengan placeholder | Periksa `products.ts` | FR-12 | **Owner** | **TERBUKA.** Dilaporkan sendiri oleh FE (D-FE-03) dan **saya konfirmasi**. Tidak menahan rilis sesuai R-13; placeholder menempati ruang 4:5 yang sama sehingga penggantian nanti tidak menggeser tata letak |
...
| **DEF-10** | Minor | Pada target ekspor statis, `trailingSlash` membuat kanonis dan `og:url` berakhiran garis miring (`.../produk/abmisibil/`) sementara `sitemap.xml` menuliskan URL tanpa garis miring. Selain itu seluruh kanonis menunjuk `titikasalkopi.id` padahal halamannya disajikan dari `github.io` | Bandingkan kanonis dan `<loc>` pada `out/` | FR-45 | **BE / Arsitek** | **TERBUKA, risiko rendah dan diterima.** Dampaknya nol selama `robots.txt` pada target itu berbunyi `Disallow: /`, yang sudah terbukti pada TC-168. Wajib ditinjau ulang bila GitHub Pages suatu saat dijadikan target produksi |
| **DEF-11** | Minor | `province` Palimping diisi "Jawa Barat" sementara brand brief hanya menulis "Desa Palimping, Garut". Nilai itu tayang pada judul metadata halaman | Bandingkan `products.ts` terhadap brand brief | FR-07, BR-01 | **BA / Owner** | **TERBUKA.** Dilaporkan sendiri oleh BE dan **saya konfirmasi**. Secara geografis benar dan bukan atribut kopi, tetapi BR-01 menyatakan brand brief adalah sumber kebenaran. Butuh satu kalimat konfirmasi owner, bukan perubahan kode |
| **DEF-12** | Minor | Pada HTML statis `/keranjang`, janji jam balas hanya hadir lewat footer. Salinan di dalam blok checkout dirender di sisi klien dan hanya muncul ketika keranjang berisi, sehingga pengunjung dengan keranjang kosong tidak melihatnya di tempat yang diminta BR-19 | Cari `08.00–21.00 WIB` pada `out/keranjang/index.html` dan hitung kemunculannya | BR-19, FR-26 | **FE** | **TERBUKA, risiko rendah.** Pengunjung yang benar-benar akan memesan pasti punya isi keranjang dan melihatnya. Perlu diverifikasi di peramban (TC-129) sebelum dinyatakan tuntas |
| **DEF-13** | Minor | `docs/05-backend.md` Bagian 11 butir 4 menyatakan pengecualian lint `SEMENTARA` untuk `src/app/page.tsx` masih ada; pengecualian itu sudah dihapus. Dokumen serah terima yang usang membuat pembaca berikutnya mengejar pekerjaan yang sudah selesai | Cari `SEMENTARA` pada `eslint.config.mjs` | Higiene dokumen | **BE** | **TERBUKA, kosmetik** |
